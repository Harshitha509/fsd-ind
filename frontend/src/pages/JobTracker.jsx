  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        // Optimistic UI Update: update state instantly instead of refetching
        setJobs(jobs.map(j => j.id === currentJob.id ? { ...j, company: currentJob.company, role: currentJob.role, status: currentJob.status } : j));
      } else {
        const { data, error } = await supabase
          .from('job_applications')
          .insert([{ 
          ...
          }]).select(); // Add .select() to get the newly generated ID
        if (error) throw error;
        
        // Optimistic UI Update: add to state instantly
